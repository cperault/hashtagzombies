...
class PlayerDB
{
    ...
    //query to update has_character field for player
    public static function update_player_after_character_creation($player_id, $character_id)
    {
        $db = Database::getDB();
        $query = 'UPDATE Players
                  SET hasCharacter = 1, characterID = :character_id WHERE playerID = :player_id';
        $statement = $db->prepare($query);
        $statement->bindValue(':player_id', $player_id);
