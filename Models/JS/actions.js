...
var checkHit = function(){
    for(var j = 0; j < x.length; j++){
        for(var i = 0; i < rounds.length; i++){
            if(rounds[i].xPos >= (x[j].xPos-15) && rounds[i].xPos<= (x[j].xPos+15) && rounds[i].yPos >= (x[j].yPos-15) && rounds[i].yPos <= (x[j].yPos+15)){
                hit = 2;
                x[j].health -= hit;
                rounds.shift();
            }
...
var surCheckHit = function(){
    for(var j = 0; j < x.length; j++){
        for(var i = 0; i < sRounds.length; i++){
            if(sRounds[i].xPos >= (x[j].xPos-15) && sRounds[i].xPos<= (x[j].xPos+15) && sRounds[i].yPos >= (x[j].yPos-15) && sRounds[i].yPos <= (x[j].yPos+15)){
                hit = 5;
                x[j].health -= hit;
                sRounds.shift();
            }
...
var how = function() {
    ...
    fill(255, 255, 255);
    textSize(20);
    text("Survive against waves of zombies. Use money to buy other survivors, weapons, and to improve your barricade. There are colored lines on the battlefield to tell you the range of weapons and other survivors.", 125, 200, 339,210);
    quite.draw();
};
